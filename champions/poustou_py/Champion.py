...
def jouer_tour():
    ...
    for troupe_id in ids:
        ...
        while troupe.pts_action > 0:
            dprint(troupe.pts_action, "restants")
            if len(chemin) > 0:
            ...
            if len(chemin) == 0:
                ...
                if target[troupe_id] is None:
                    dprint("Not found, exit !")
                    break
                arrivee, chemin = target[troupe_id]
                if len(chemin) == 0:
                    dprint("Nothing to do, exit !")
                    break
            troupe = get_troupe_by_id(troupe_id)
